#!/usr/bin/env python3
"""Generate Fig. 5: central gap, velocity, and force time series."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate central gap / velocity / force time series (Fig. 5)."
    )
    ...
    with h5py.File(h5_file, "r") as f:
        for j, n in enumerate(N_COLS):
            ...
            for St_parsed, stokes_key in stokes_pairs:
                ...
                if "Central_h" not in grp or "V" not in grp or "Force" not in grp:
                    continue

                Central_h = np.array(grp["Central_h"])
                V = np.array(grp["V"])
                ...
                axes[0, j].plot(t_snaps, Central_h, color=color, lw=1.5)
                axes[1, j].plot(t_snaps, V, color=color, lw=1.5)
                axes[2, j].plot(t_snaps, F, color=color, lw=1.5)
    ...
    axes[0, 0].set_ylabel(r"$\tilde{h}$", rotation=0, labelpad=15, fontsize=25)
    axes[1, 0].set_ylabel(r"$\tilde{V}$", rotation=0, labelpad=15, fontsize=25)
    axes[2, 0].set_ylabel(r"$\tilde{F}$", rotation=0, labelpad=15, fontsize=25)

    axes[0, 0].set_ylim(-0.01, 0.25)
    axes[1, 0].set_ylim(-1.1, 1.1)
    axes[2, 0].set_ylim(-0.2, 1.4)
    ...
    output = output_path("time_series.pdf", args.output)
